...
void draw_pdf(int tof_option, int energy_option, double true_mom) // GeV/c
{
  int fixed_mom = (int)((true_mom - 0.3) * 1000.);
  // Open the root file
  TFile *file = new TFile(Form("/home/had/kohki/work/ML/Likelihood/buf/pdf_tof%dene%d.root", tof_option, energy_option));
  ...
  // Close the file
  // file->Close();
  canvas->SaveAs(Form("/home/had/kohki/work/ML/Likelihood/ana/fig/pdf_tof%dene%d_mom%f.png", tof_option, energy_option, true_mom));
}
