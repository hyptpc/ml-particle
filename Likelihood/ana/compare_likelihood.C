// root 'compare_likelihood.C(<tof分解能>, <エネルギー分解能>)'で実行
// likelihoodとMLのeff, fomの比較を行う
// 最終更新日: 2024/02/20
...
void compare_likelihood(int tof_option, int energy_option)
{
  gStyle->SetOptStat(0);
  TFile *fin1 = new TFile(Form("/home/had/kohki/work/ML/Likelihood/buf/pdf_tof%dene%d.root", tof_option, energy_option));
  TFile *fin2 = new TFile("/home/had/kohki/work/ML/Likelihood/buf/test_Kppi_TofMom.root");
  TFile *fin3 = new TFile(Form("/home/had/kohki/work/ML/create_rootfiles/output_digits_tof%d_ene%d.root", tof_option, energy_option));
  // TFile *fin3 = new TFile(Form("/home/had/kohki/work/ML/create_rootfiles/output_bigdata_tof%d_ene%d.root", tof_option, energy_option));
  ...
  double particle;
  int particle_ML_input, particle_ML_output;
  double mom;
  double tofres; // tof of 40cm distance
  double energy_lossres;

  t2->SetBranchAddress("particle", &particle);
  t2->SetBranchAddress("mom", &mom);
  t2->SetBranchAddress(Form("tofres%d", tof_option), &tofres);
  t2->SetBranchAddress(Form("energyres%d", energy_option), &energy_lossres);
  ...
  for (int k = 0; k < ev_max; k++)
  {
    ...
    int tof_id = hpdf[0][mom_id]->GetXaxis()->FindBin(tofres);
    int Eloss_id = hpdf[0][mom_id]->GetYaxis()->FindBin(energy_lossres);
    double pdf[3];
    double Likeli[3];
    for (int ip = 0; ip < 3; ++ip)
    {
  ...
  double EffML[3];
  double FoMML[3];
  t3->SetBranchAddress("particle", &particle_ML_input);
  t3->SetBranchAddress("particle_ML", &particle_ML_output);
  for (int ip = 0; ip < 3; ++ip)
  {
    ...
    {
      t3->GetEntry(k);
      if ((int)particle_ML_input == ip)
      {
        count_particle++;
        if ((int)particle_ML_output == ip)
        {
          count_match++;
        }
      }
  ...
  for (int ip = 0; ip < 3; ++ip)
  {
    ...
    point->Draw("P");
    std::cout << "Eff : " << EffML[ip] << " FoM : " << FoMML[ip] << std::endl;
  }
  c->SaveAs(Form("/home/had/kohki/work/ML/Likelihood/ana/fig/CompareLikelihood_tof%dene%d.png", tof_option, energy_option));
}
